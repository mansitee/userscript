...
(function() {
    ...
    GM_addStyle(`
.fm{background:rgba(255,255,255, 0.5);}
.floating{background:#007bff;}
.fm .mg,.fm .mgs, .fm .copy, .fm button:hover {background: rgba(0,123,255,0.1);}
.floating, .fm, .fm .glink, .fm a, .fm button, .fm button:hover {-webkit-tap-highlight-color: transparent;}
.floating, .fm .glink, .fm .copy {text-align:center;}
.fm a:hover, .fm .copy, .fm .glink {font-weight:bold;color:#007bff;}


.floating {color:#fff;font-size: 30px; border-radius:50%; width: 40px; height:40px; line-height: 40px; position:fixed; bottom:0px; right:6%; cursor:move; z-index: 999999999; animation:3s linear 0s 1 normal none running fadeIn; transition: bottom 0.2s; }
.floating:hover {color:#fff;}

.fm {display:none;position:fixed; bottom:80px;right:20px;border-radius:5px; -webkit-backdrop-filter: blur(8px); backdrop-filter: blur(8px); z-index:999999999;}
.fm:hover {opacity:1; opacity 0.2s ease;transform: translate(-50px); transition: transform 0.5s ease,}
...
.fm .mgs svg, .fm .mgs a {font-size:12px;color:#007bff;font-family: Arial, sans-serif; 
    font-weight: bold;
     text-decoration: none;
    text-transform: uppercase;padding:10px 0 10px 0;text-align: center; border-radius:10px;}
...
.fm a:last-child, .fm button:last-child {border-bottom: none;}
        .fm .copy { opacity: 0.8; padding: 10px; width: 100%; border-radius: 30px 30px 0 0; border-top: 2px solid white; }
    `);
    ...
    const menu = $(`<div class="fm">
        <a href="https://www.google.com/search?hl=id&safe=off&q=${encodeURIComponent(window.location.href)}" target="_blank">• Google No</a>
